<template>
    <div class="flex flex-col min-h-screen">
        <div :class="{ 'bg-white shadow-xl border border-b-amber-50': scrolled, 'bg-transparent': !scrolled }"
            class="hidden sm:block header fixed w-full top-0 left-0 z-50 transition-colors duration-300">
            <div class="container mx-auto flex items-center justify-between py-4">
                <div class="logo">
                    <a href="#hero">
                        <span class="text-white font-bold text-2xl" :class="{ 'text-custom-green': scrolled }">TOP
                            RACKET</span>
                    </a>
                </div>
                <nav class="flex space-x-4">
                    <a v-for="link in navLinks" :key="link.text" :href="link.href"
                        :class="{ 'text-custom-green hover:text-yellow-500': scrolled }" class="text-white">
                        <span>{{ link.text }}</span>
                    </a>
        ...
        <div class="sm:hidden">
            ...
            <div class="text-center bg-blue-100 transition-all duration-300 ease-in-out"
        ...
        <footer class="bg-gray-900 p-4 sm:py-8 text-white">
            <section-footer />
        </footer>
    </div>
</template>
